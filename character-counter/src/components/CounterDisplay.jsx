function CounterDisplay({ text, includeSpaces, charLimit }) {
  ...
  return (
    <div className="w-full mb-8 grid grid-cols-1 sm:grid-cols-2 gap-4 py-6">
      <div className="p-6 bg-[var(--color-purple-400)] dark:bg-gray-800  rounded-lg shadow-md flex justify-between items-center">
        <p className={charCount > charLimit ? "text-red-600 font-bold text-4xl" : "text-4xl font-bold text-[var(--color-text)] dark:text-gray-200"}>
         {charCount} <span className={charCount > charLimit ? "text-[var(--color-text) font-bold text-4xl" : "text-4xl text-[var(--color-text)] dark:text-gray-200"}>/ {charLimit} </span>  <span className={charCount > charLimit ? "text-[var(--color-text) font-medium text-2xl" : "font-medium text-2xl dark:text-gray-200"}> CHARACTERS</span>
        </p>
      ...
      <div className="p-6 bg-[var(--color-neutral-200)] dark:bg-gray-800 rounded-lg shadow-md flex justify-between items-center">
        ...
        <svg className="w-48 h-48 text-[var(--color-neutral-700)] translate-x-5" fill="none" stroke="var(--color-neutral-100)" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
